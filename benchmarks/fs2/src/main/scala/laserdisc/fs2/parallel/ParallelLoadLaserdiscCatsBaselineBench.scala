...
import laserdisc.fs2.parallel.baseline.BenchRedisBaselineChannel
  ...
  class LaserdiscCatsBaselineSetUp {
    ...
    val resource = Blocker[IO] evalMap { bl =>
      RedisAddress("localhost", 6379).toInetSocketAddress[IO] map { address =>
        BenchRedisBaselineChannel[IO](address, writeTimeout = Some(10.seconds), readMaxBytes = 8 * 1024 * 1024)(bl)
      }
    }
