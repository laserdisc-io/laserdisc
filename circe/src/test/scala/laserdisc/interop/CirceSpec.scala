...
final class CirceSpec extends WordSpec with MustMatchers with PropertyChecks with OptionValues {
  ...
  private[this] implicit val barGen: Gen[Bar] = Arbitrary.arbitrary[Int].map(Bar.apply)
  private[this] implicit val bazGen: Gen[Baz] = for {
    s   <- Arbitrary.arbitrary[String]
    foo <- fooGen
  } yield Baz(s, foo)
  private[this] implicit def fooGen: Gen[Foo] = Gen.oneOf(barGen, bazGen)
