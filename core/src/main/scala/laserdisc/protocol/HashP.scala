...
trait HashP {
  ...
  final def hdel(key: Key, fields: OneOrMoreKeys): Protocol.Aux[NonNegInt] =
    Protocol("HDEL", key :: fields.value).as[Num, NonNegInt]

  final def hexists(key: Key, field: Key): Protocol.Aux[Boolean] =
    Protocol("HEXISTS", key :: field :: Nil).as[Num, Boolean]
  ...
  final def hgetall[A](key: Key)(
      implicit ev: Arr ==> A
  ): Protocol.Aux[A] = Protocol("HGETALL", key).as[Arr, A]
  ...
  final def hmget[L <: HList](key: Key, fields: OneOrMoreKeys)(
      implicit ev: Arr ==> L
  ): Protocol.Aux[L] = Protocol("HMGET", key :: fields.value).as[Arr, L]
  ...
  final def hscan(key: Key, cursor: NonNegLong): Protocol.Aux[ScanKV] =
    Protocol("HSCAN", key :: cursor :: HNil).as[Arr, ScanKV]
  ...
  final def hstrlen(key: Key, field: Key): Protocol.Aux[NonNegInt] =
    Protocol("HSTRLEN", key :: field :: Nil).as[Num, NonNegInt]

  final def hvals[L <: HList](key: Key)(
      implicit ev: Arr ==> L
  ): Protocol.Aux[L] = Protocol("HVALS", key).as[Arr, L]
}
