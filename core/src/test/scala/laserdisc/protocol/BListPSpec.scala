  ...
  "The Blocking List protocol" when {
    ...
    "using blpop" should {
      ...
      "roundtrip successfully" when {
        "given one or more keys and timeout" in forAll("keys", "timeout", "return value") { (ks: OneOrMoreKeys, nni: NonNegInt, i: Int) =>
    ...
    "using brpoplpush" should {
      ...
      "roundtrip successfully" when {
        "given source key and destination key" in forAll("sourcekey", "destinationkey", "return value") { (s: Key, d: Key, i: Int) =>
        ...
        "given source key, destination key and specific read instance" in {
          forAll("sourcekey", "destinationkey", "return value") { (s: Key, d: Key, i: Int) =>
        ...
        "given source key, destination key and timeout" in {
          forAll("sourcekey", "destinationkey", "timeout", "return value") { (s: Key, d: Key, pi: PosInt, i: Int) =>
