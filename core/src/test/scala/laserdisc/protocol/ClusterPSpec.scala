    ...
    "using slots" should {
      "roundtrip successfully" when {
        "using val" in forAll(slotsGen) { ss =>
          // even if empty lists should never be generated by nonEmptyListOf
          // the whenever barrier is needed because of
          // https://github.com/typelevel/scalacheck/issues/443
          whenever(ss.forall(_._3.nonEmpty)) {
            val protocol = slots
